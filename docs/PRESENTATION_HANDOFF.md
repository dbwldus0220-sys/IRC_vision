...
초기 OpenCV 색상·윤곽선 방식은 조명, 그림자, 바닥 색상과 배경 변화에 민감했습니다. 이를 개선하기 위해 한 YOLO26 모델에서 `line`, `ball`, `goal`, `backboard`, `hurdle`, `grab`을 동시에 탐지하고 RealSense depth를 결합했습니다. `grab`은 공을 잡은 상태를 확인하는 검출 클래스이며, 현재는 제어 입력이 아닌 시각 진단 용도로 사용합니다.
